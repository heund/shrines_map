...
<body>
  <div class="shell">
    <header class="header">
      <div class="header-title">
        <h1>제주비엔날레 장소 지도</h1>
        <p>문화유산, 비엔날레 장소, 신당 레이어를 한 화면에서 비교합니다.</p>
      </div>
    ...
    <div class="content">
      <aside class="sidebar" id="sidebar">
        <div class="sidebar-head">
          <h2>레이어 제어</h2>
          <p>문화유산은 지정 유형별 색으로 구분하고, 신당은 소유권 기준으로 분류합니다. 좌표가 없는 신당 27건은 목록에서만 집계하고 지도에는 표시하지 않습니다.</p>
        </div>
        <div class="sidebar-scroll">
          <details class="section" open id="venuesSection">
            ...
            <div class="section-body">
              ...
              <p class="section-note">비엔날레 장소는 별도 하위 유형 없이 하나의 강조 레이어로 표시합니다.</p>
            </div>
          </details>
          ...
          <details class="section" open id="basemapSection">
            ...
            <div class="section-body">
              ...
              <p class="section-note">Google 기본 지도에 포함된 POI 및 랜드마크 표시를 켜거나 끌 수 있습니다.</p>
            </div>
          </details>
        </div>
      </aside>
  ...
  <script>
    ...
    function createSvgMarker(shape, color, size) {
      let svg = "";
      if (shape === "diamond") {
        ...
        svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 40 40"><path d="M20 3.5c6.8 0 12.3 5.4 12.3 12.1 0 8.8-8.8 13.3-12.3 20.9-3.5-7.6-12.3-12.1-12.3-20.9C7.7 8.9 13.2 3.5 20 3.5z" fill="${color}" stroke="rgba(34,27,22,0.68)" stroke-width="1.5"/><path d="M12.5 16.5h15" stroke="rgba(255,255,255,0.9)" stroke-width="1.6" stroke-linecap="round"/><path d="M15 12.8h10" stroke="rgba(255,255,255,0.85)" stroke-width="1.4" stroke-linecap="round"/><path d="M20 11.5v10.5" stroke="rgba(255,255,255,0.85)" stroke-width="1.4" stroke-linecap="round"/></svg>`;
      } else {
    ...
    function buildPopupCard({ title, badges = [], image = null, body = "", footer = "" }) {
    ...
    function heritageBody(item) {
      return item.description
        ? `<div class="popup-text"><details><summary>설명 펼치기</summary>${escapeHtml(item.description)}</details></div>`
    ...
    function shrineBody(item) {
      ...
      return `
        ...
        <div class="popup-meta">
          <span class="badge">${escapeHtml(item.ownership_standardized || "미상")}</span>
    ...
    function createMarker(shape, color, size, position, title) {
      return new google.maps.Marker({
        position,
        map,
        title,
        icon: {
          url: createSvgMarker(shape, color, size),
    ...
    function createShrineMarkers() {
      mapData.shrines
        ...
        .forEach((item) => {
          ...
          const marker = createMarker("shrine", color, 28, position, item.shrine_name);
          setPopup(marker, buildPopupCard({
            title: item.shrine_name,
            badges: [{ label: owner }],
            body: shrineBody(item)
          }));
          state.markers.shrines.push({ marker, owner });
        });
    }
    ...
    function refreshMarkerVisibility() {
      ...
      state.markers.shrines.forEach(({ marker, owner }) => {
        const visible = (
    ...
    function renderSidebar() {
      ...
      document.getElementById("header-counts").textContent = `문화유산 ${mapData.heritage.length} · 장소 ${mapData.venues.length} · 신당 ${mapData.shrines.length}`;
    ...
    function syncMasterToggle(sectionName) {
      if (sectionName === "heritage") {
        ...
        toggle.checked = allOwners.every((owner) => state.shrineOwners.has(owner));
        toggle.indeterminate = state.shrineOwners.size > 0 && state.shrineOwners.size < allOwners.length;
      }
    }
